...
5. Start Celery Beat.
6. Open the frontend application in the browser.
7. Login as Admin or register as a Student or Company.
8. The Admin approves registered companies.
9. Approved companies create placement drives.
10. The Admin approves placement drives.
11. Students view approved drives and apply based on CGPA eligibility.
12. Companies manage applicants and update their application status.
13. Students can track their application and placement history.
...
The application sends email reminders to students about upcoming placement drive deadlines.

The task is scheduled using Celery Beat and runs automatically at the configured time.

## Monthly Activity Report

A monthly placement activity report is generated automatically on the first day of every month and sent to the Admin through email.
...
Students can trigger an asynchronous CSV export of their placement application history from the Student Dashboard.

The export is processed by Celery, and the student receives an alert when the file is ready for download.
